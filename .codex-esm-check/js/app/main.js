...
function cacheDom() {
  ...
  dom.btnOpenDocumentSubmission = document.getElementById('btnOpenDocumentSubmission');
  dom.btnStartReservation = document.getElementById('btnStartReservation');
  dom.documentSubmissionModal = document.getElementById('documentSubmissionModal');
  dom.documentSubmissionForm = document.getElementById('documentSubmissionForm');
  ...
  dom.documentSuccessModal = document.getElementById('documentSuccessModal');
}
...
function bindStaticEvents() {
  ...
  dom.documentSubmissionForm.addEventListener('submit', submitDocument);
...
function openDocumentSubmissionModal() {
  const event = getEvent(getActionEventId());
  if (!event?.documentTemplateUrl) {
    alert('등록된 제출 양식이 없습니다.');
    return;
  }
...
async function submitDocument(event) {
  ...
  const currentEvent = getEvent(getActionEventId());
  const file = dom.docFile.files?.[0];

  if (!currentEvent?.documentTemplateUrl) {
    showDocumentError('등록된 제출 양식이 없습니다.');
    return;
  }

  if (!file) {
    showDocumentError('업로드할 파일을 선택해 주세요.');
    return;
  }

  if (file.size > MAX_DOCUMENT_FILE_SIZE_MB * 1024 * 1024) {
    showDocumentError(`파일 크기는 ${MAX_DOCUMENT_FILE_SIZE_MB}MB 이하만 업로드할 수 있습니다.`);
    return;
  }

  hideDocumentError();
  dom.btnSubmitDocument.disabled = true;
  dom.btnSubmitDocument.textContent = '제출중...';
  ...
  try {
    ...
    console.error('Document submission failed, storing locally:', error);
    const submissions = JSON.parse(localStorage.getItem('document_submissions') || '[]');
    submissions.push({
      ...payload,
      id: `doc_${Date.now()}`,
      fileUrl: '',
  ...
  dom.documentSubmissionModal.classList.remove('active');
  dom.eventActionModal.classList.remove('active');
  dom.documentSuccessModal.classList.add('active');
}

function showDocumentError(message) {
  dom.docError.textContent = message;
  dom.docError.style.display = 'block';
}

function hideDocumentError() {
  dom.docError.textContent = '';
  dom.docError.style.display = 'none';
}
...
function guessMimeType(fileName) {
  ...
  if (normalized.endsWith('.hwpx')) return 'application/x-hwpx';
  if (normalized.endsWith('.hwp')) return 'application/x-hwp';
...
function resolveTemplateDownloadUrl(url) {
  const raw = String(url || '').trim();
  const githubBlobMatch = raw.match(/^https:\/\/github\.com\/([^/]+)\/([^/]+)\/blob\/(.+)$/i);
  if (!githubBlobMatch) return raw;
